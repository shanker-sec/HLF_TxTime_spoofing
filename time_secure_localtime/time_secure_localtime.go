...
func (t *TimeStampChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "CheckDividents_secure_localtime":
		return t.CheckDividents_secure_localtime(stub)
	case "Stake_secure_localtime":
		return t.Stake_secure_localtime(stub, args)
	default:
		return shim.Error("Invalid function name.")
	}
}
//deposit
func (t *TimeStampChaincode) Stake_secure_localtime(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
	...
	max_time_diff := 300
	diff := int(math.Abs(txTime.Sub(timeNow).Abs().Seconds()))
...
func (t *TimeStampChaincode) subtractTimestamp(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if savedTimestamp == nil {
		return shim.Error(fmt.Sprintf("you must first call the Stake_secure_localtime() function"))
	}
...
func (t *TimeStampChaincode) CheckDividents_secure_localtime(stub shim.ChaincodeStubInterface) pb.Response {
