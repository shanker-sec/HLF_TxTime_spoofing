...
func (t *TimeStampChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case "CheckDividents_secure_nts":
		return t.CheckDividents_secure_nts(stub)
	case "Stake_secure_nts":
		return t.Stake_secure_nts(stub, args)
	default:
		return shim.Error("Invalid function name.")
	}
}
//deposit
func (t *TimeStampChaincode) Stake_secure_nts(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
	...
	session, err := nts.NewSession(nts_server)
	if err != nil {
		return shim.Error(fmt.Sprintf("Failed to connect to NTS server: %v", err))
	}
	...
	return shim.Error(fmt.Sprintf("Failed to get response from NTS server: %v", err))
...
func (t *TimeStampChaincode) subtractTimestamp(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if savedTimestamp == nil {
		return shim.Error(fmt.Sprintf("you must first call the Stake_secure_nts() function"))
	}
...
func (t *TimeStampChaincode) CheckDividents_secure_nts(stub shim.ChaincodeStubInterface) pb.Response {
